...
def record_face():
    def capture_face():
        ...
        def process_frame():
            ...
            for (top, right, bottom, left) in faces:
                ...
                file_path = os.path.join("faces", f"{name}_{nik}_{count}.jpg")
            ...
            if count < 10:
                ...
                cursor.execute("INSERT INTO users (name, nik, image_path) VALUES (%s, %s, %s)", (name, nik, file_path))
                db.commit()
    ...
    tk.Label(record_window, text="NIK").pack(pady=10)
    nik_entry = tk.Entry(record_window)
...
def mark_attendance():
    ...
    cursor.execute("SELECT id, name, nik, image_path FROM users")
    known_encodings = []
    known_names = []
    known_user_ids = []
    for user_id, name, nik, image_path in cursor.fetchall():
...
def view_report():
    ...
    tree = ttk.Treeview(report_window, columns=("No", "Nama", "NIK", "Tanggal", "Waktu"), show="headings")
    ...
    cursor.execute("""
        SELECT ROW_NUMBER() OVER (ORDER BY a.date, a.time) AS no, u.name, u.nik, a.date, a.time 
        FROM attendance a 
...
def create_main_menu():
    ...
    def add_menu_button(text, command, bg_color, fg_color):
        btn = tk.Button(
            menu_frame,
            text=text,
            command=command,
    ...
    add_menu_button("Rekam Wajah", record_face, "#007bff", "white")
    add_menu_button("Absensi", mark_attendance, "#28a745", "white")
    add_menu_button("Laporan", view_report, "#ffc107", "black")
    add_menu_button("Keluar", root.destroy, "#dc3545", "white")
